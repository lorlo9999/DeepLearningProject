...
svc = OneVsRestClassifier(svm.SVC( decision_function_shape='ovo',probability=True, kernel='linear')) #svm.SVC(kernel='linear', probability=True, random_state=random_state)
...
for i in range(4):
    for _ in range(n_bootstraps):
        ...
        y_prob_resampled = svc.predict_proba(X_resampled)
...
plt.savefig('../plots/SVC_ROC.pdf', dpi=400)
plt.show()
